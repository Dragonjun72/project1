...
int main_pointer_project(void)
{
	...
	long preElapesedTime = 0;		// 직전 경과 시간 (최근 물 준 시간 간격)
	...
	startTime = clock(); // 현재 시간을 millisecond(1000분의1초) 단위로 반환
	while(1)
	{	
		...
		// 사용자가 입력한 어항에 물을 준다
		// 1. 어항의 물이 0 이면? 물을 주지 않는다... 이미 고기가...
		if(cursor[num-1]<=0)
		{
		...
		// 10초 -> 15초 (5초 preElapesedTime -> 15초) -> 25초
	}
	return 0;
}
